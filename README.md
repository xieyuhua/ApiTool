...
| `db_schema` | 🗄️ | **数据库连接分析**：读取已配置连接的表结构（库/表/字段名/类型/可空/默认值/注释），**叠加用户维护的表级/字段级语义**，整理为 Markdown 表格供模型理解结构 | `connId`（可省略，用激活连接）、`database`、`tables`（可选） |
| `db_query` | 🔎 | **数据库连接分析**：在已配置连接上执行只读 `SELECT`/`WITH` 查询并返回结果（限行），用于采样数据辅助分析**禁止任何写操作** | `connId`（可省略）、`database`、`sql`、`limit`（可选） |
