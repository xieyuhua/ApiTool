...
> 说明：剪贴板历史浮层可通过系统托盘菜单「剪贴板历史…」打开，无需额外设置。
...
| `db_schema` | 🗄️ | **数据库连接分析**：返回用户在「插件/数据库连接」中**已同步配置的**表结构（库/表/字段名/类型/可空/默认值/注释），**叠加用户维护的表级/字段级语义**，整理为 Markdown 表格供模型理解结构。**只返回你已同步的表，不会实时读取全库**，模型应严格只基于返回的表/字段生成 SQL | `connId`（可省略，用激活连接）、`database`、`tables`（可选，不填则返回该库下所有已同步表） |
| `db_query` | 🔎 | **数据库连接分析**：在已配置连接上执行只读 `SELECT`/`WITH` 查询并返回结果（限行），用于采样数据辅助分析**禁止任何写操作**；**额外校验**：SQL 只能引用你已同步配置的表（`db_schema` 返回的表），引用未同步表会直接拒绝，确保模型只基于你提供的表/字段生成 SQL | `connId`（可省略）、`database`、`sql`、`limit`（可选） |

> 说明：`web_search` 依赖访问外网 DuckDuckGo，离线 / 网络受限环境可能失败；`run_command` 会在本机执行真实命令，使用前请确认信任当前对话内容。

##### 数据库连接分析（MySQL / PostgreSQL / Oracle）

Agent 内置两个数据库工具，让大模型直接「看到」数据库结构与数据，从而做数据分析、SQL 生成、字段语义理解：

1. **`db_schema`**：把指定库（或全部表）的**表结构同步**给模型——字段名、类型、是否可空、默认值、以及数据库自带的 `COMMENT` 注释。**输出还会叠加用户在「插件 → 数据库连接」里维护的「表级 / 字段级语义」**（见下文），这部分比数据库自带注释更贴近业务，模型会优先采用。
   - `tables` 留空 = 同步该库全部表；指定若干表名 = 仅同步选中的表（也支持在对话里让模型「只分析订单表」）。
   - `connId` 可省略：省略时自动使用「当前激活的分析连接」。
2. **`db_query`**：执行只读 `SELECT`/`WITH` 查询并采样返回结果（默认上限 200 行），供模型验证假设、查看样例数据。**禁止任何写操作**（INSERT/UPDATE/DELETE/DDL 会被拦截）。`connId` 同样可省略（用激活连接）。

**工作流（配置 → 启用 → 使用）**：
1. **配置连接**：进入 **「插件 → 数据库连接」**，新增 MySQL / PostgreSQL / Oracle 连接（连接 ID 形如 `db_<时间戳>`，如 `db_1788254096769`）。保存后点「测试连接」验证可用性。
2. **启用分析连接（唯一）**：在连接详情里打开「启用为分析连接」——**同时只能有一个连接被启用为分析连接**，启用新的会自动替换旧的。该连接即作为 `db_schema`/`db_query` 省略 `connId` 时的默认连接。
3. **同步表结构**：选中库（Schema）后，勾选需要同步的表，点「同步选中表结构」。`diygw` 等库名大小写、表名大小写均自动适配（MySQL 已做 `DATABASE()` 容错）。
4. **维护语义（表级 + 字段级）**：
   - 在「已同步表结构」里，每张表可填**表语义**（整张表的业务含义）。
   - 展开表后，每个字段可填**中文语义**（如「订单创建时间」）。
   - 每个输入框旁有 **「AI 生成」** 按钮：调用已配置的 AI 接口，根据表名 / 字段名 / 类型 / 已有注释一键生成中文业务描述，生成后自动保存。
   - 维护内容**实时自动保存**到 `agent.json`，无需手动保存按钮（卡片标题栏有绿色「修改后自动保存」标记）。
5. **持久化与恢复**：关闭并重新打开该连接时，会**自动恢复上次选中的库**；已同步的表结构与表/字段语义全部保留。每个连接各自记住上次选中的库（`agent.json` 的 `dbLastDB`）。
6. **启用工具**：在 Agent 设置页的「数据库连接分析」分组中**启用** `db_schema` / `db_query` 两个内置工具。
7. **对话使用**：直接说「分析一下 xxx 库的订单表结构」「查一下最近 10 条订单」，模型会自动调用对应工具（带语义的结构会回灌给模型）。

> 说明：`db_schema` / `db_query` 的结果在对话里**默认完整展开显示**（参数 + 返回），便于核对；若执行报错会明确提示（如库名 / 表名大小写不匹配、权限不足等）。

**系统提示词（兜底通用助手）**：Agent 默认系统提示词是一个**通用智能助手**——会严格按你在对话中给定的角色行事（数据分析师 / 运维工程师 / 客服 / 开发等），未指定角色时以「贴心、专业、严谨」的通用助手身份回答。你可以在 **Agent 设置 → 运行配置 → 系统提示词** 中改写为任意角色说明（留空则用内置通用助手默认）。数据库相关准则（优先用工具、只读 SELECT、优先采用你维护的字段语义）始终生效，不随角色改变。
...
#### 13.7 预置数据分析模板技能（Skill）

Agent 内置 4 个**数据分析模板技能**（在「设置 → Agent → 技能(Skill)」中可见，默认启用，可编辑 / 关闭 / 新增）：

| 技能 | 触发场景 | 核心动作 |
| --- | --- | --- |
| **销售数据分析** | 销售额 / 销量 / 订单 / 营收 / 客单价 / 最近一周·月·季度销售 | `db_schema` 找订单/商品表 → `db_query` 按时间/品类/地区/渠道聚合 → 算环比同比 → Top 商品与异常波动 |
| **用户留存分析** | 新增 / 活跃(DAU/WAU/MAU) / 留存率 / 流失率 / 复购 / 生命周期 | `db_schema` 定位用户/行为/订单表 → `db_query` 算留存与同期群(Cohort) → 渠道对比 |
| **流量转化分析** | 访问量 / 转化率 / 漏斗 / 加购 / 下单转化 / 渠道效果 | `db_schema` 定位各漏斗环节表 → `db_query` 构建漏斗(曝光→点击→加购→下单→支付) → 定位流失环节 |
| **指标异常诊断** | 某指标突增/突降 / 数据异常 / 波动原因 / 对比上期 | `db_schema` 确认指标表 → `db_query` 拉异常期与对照期分维度数据 → 下钻定位根因 |

- 技能（名称 + 描述 + 具体指引）会在运行时注入系统提示词；模型遇到匹配场景时**先说明将使用哪个技能，再按指引分步骤调用 `db_schema` / `db_query` 取数分析**。
- 预置技能通过 `mergeDefaultSkills` 在加载时与你的自定义技能合并，**不会覆盖你在设置里修改或新增的技能**（同名 ID 以你的为准）。
- 若你删除了某个预置技能，升级程序后不会自动加回（避免覆盖你的意图）。
