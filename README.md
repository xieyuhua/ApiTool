...
| `db_schema` | 🗄️ | **数据库连接分析**：读取已配置连接的表结构（库/表/字段名/类型/可空/默认值/注释），整理为 Markdown 表格，供大模型理解数据结构 | `connId`、`database`、`tables`（可选） |
| `db_query` | 🔎 | **数据库连接分析**：在已配置连接上执行只读 `SELECT` 查询并返回结果（限行），用于采样数据辅助分析 | `connId`、`database`、`sql`、`limit`（可选） |
...
1. **`db_schema`**：把指定库（或全部表）的**表结构同步**给模型——字段名、类型、是否可空、默认值、以及数据库自带的 `COMMENT` 注释（作为「字段语义」展示）。输出为 Markdown 表格，模型可直接据此理解表含义、字段业务语义。
   - `tables` 留空 = 同步该库全部表；指定若干表名 = 仅同步选中的表（也支持在对话里让模型「只分析订单表」）。
2. **`db_query`**：执行只读 `SELECT`/`WITH` 查询并采样返回结果（默认上限 200 行），供模型验证假设、查看样例数据。**禁止任何写操作**（INSERT/UPDATE/DELETE/DDL 会被拦截）。

使用前提：先在 **「插件 → 数据库连接」** 中配置并保存 MySQL / PostgreSQL / Oracle 连接（连接 ID 形如 `mysql-1` / `pg-1` / `ora-1`），然后在 Agent 设置页的「数据库连接分析」分组中**启用**这两个工具。对话时直接说「分析一下 xxx 库的订单表结构」「查一下最近 10 条订单」，模型会自动调用对应工具。
